...
class HighPrecisionTimeEdit( wx.TextCtrl ):
	...
	def GetSeconds( self ):
		v = super(HighPrecisionTimeEdit, self).GetValue().strip().replace(' ',':').replace('-',':')
	...
	def SetSeconds( self, secs ):
		if secs is None and self.allow_none:
			...
			super(HighPrecisionTimeEdit, self).SetValue( Utils.SecondsToStr(secs or 0) )
			
	def GetValue( self ):
		self.SetSeconds( self.GetSeconds() )
		return super(HighPrecisionTimeEdit, self).GetValue()
